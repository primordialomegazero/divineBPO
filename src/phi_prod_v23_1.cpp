...
int main() {
    ...
    log.info("══════ DIVINE BPO v23.1 — PRODUCTION UPGRADES ══════");
    ...
    log.info("Rate Limit: 100 req/s per IP");
    ...
    if (pqc.init()) {
        ...
        log.info("PQC: " + g_metrics.pqc_algorithm + " (" + std::to_string(g_metrics.pqc_security_bits) + "-bit)");
    }
    ...
    log.info("API v1: http://localhost:8092/api/v1/*");
    log.info("DB Pool: 8 connections");
    log.info("Cache: " + std::string(cache.is_available() ? "Redis" : "None"));
